...
module.exports = (srf, options = {}) => {
  ...
  app.use(express.static(opts.media));
  ...
  app.get('/', (req, res) => {
    ...
    const dueNow = srf.getCountCardsDue();
    ...
    const averageStudyTime = srf.getAverageStudyTimePerDay();
    const averageLatency = srf.getAverageLatency();

    const averageNewCards = srf.getAverageNewCardsPerDay();
    const chartStudyTime = srf.getChartStudyTime();
    ...
    statsPast24Hours.time = Math.round(statsPast24Hours.time / 60);
    statsNext24Hours.time = Math.round(statsNext24Hours.time / 60);
    res.render('home', {
      refresh: 300,
      studyTime: studyTime,
      statsPast24Hours: statsPast24Hours,
      statsNext24Hours: statsNext24Hours,
      averageLatency: srf.formatSeconds(averageLatency),
      averageStudyTime: Math.round(averageStudyTime / 60),
      ...
      timeToNextDue: nextDue - now,
      studyNow: studyNow,
      chartStudyTime: chartStudyTime,
      mode: mode,
      theme: config.theme,
    });
  });
  ...
  app.get('/homeData', (req, res) => {
    ...
    const statsNext24Hours = srf.getStatsNext24Hours();
    const averageStudyTime = srf.getAverageStudyTimePerDay();
    const predictedStudyTime = Math.round(
      (
        (statsPast24Hours.time + statsNext24Hours.time) / 2 +
        averageStudyTime
      ) / 2
    ...
    res.json({
      predictedStudyTime: predictedStudyTime,
      statsPast24Hours: statsPast24Hours,
      statsNext24Hours: statsNext24Hours,
      averageLatency: srf.getAverageLatency(),
      averageStudyTime: averageStudyTime,
      averageNewCards: srf.getAverageNewCardsPerDay(),
      newCardsToday: srf.getCountNewCardsToday(),
      studyTimeToday: srf.getStudyTimeToday(),
      percentCorrect: srf.getPercentCorrect(),
      dueNow: srf.getCountCardsDue(),
      overdue: srf.getCountCardsOverdue(),
      timeToNextDue: nextDue - now,
      nextCardID: nextCard ? nextCard.id : 0,
      chartStudyTime: srf.getChartStudyTime(),
      mode: getMode(statsPast24Hours, statsNext24Hours),
      theme: srf.getConfig().theme,
    });
  });
  ...
  app.get('/stats', (req, res) => {
    ...
    charts.chartsLatency = srf.getChartsLatency();
    ...
    res.json({
      ...
      averageStudyTimePerDay: averageStudyTime,
      averageStudyTimePerCard: srf.getAverageStudyTimePerCard().toFixed(1),
      averageReviewsPerCard: srf.getAverageReviewsPerCard(),
      averageReviewsPerDay: srf.getAverageReviewsPerDay(),
      timeToNextDue: nextDue - now,
      charts: charts,
      theme: config.theme,
      statsNext24Hours: statsNext24Hours,
  ...
  app.get('/next', (req, res) => {
    ...
    res.json({
      nextURL: (card ? '/card/' + card.id + '/front' : '/'),
  ...
  app.get('/card/:id', (req, res) => {
    ...
    if (card) {
      ...
      card.timeSinceLastReview = timeCardLastSeen ? now - timeCardLastSeen : 0;
      card.intervals = srf.getIntervals(card);
      card.chartData = srf.getChartCardHistory(card.id);
      // For signal
      card.theme = srf.getConfig().theme;
      card.dueNow = srf.getCountCardsDue();
      card.statsPast24Hours = srf.getStatsPast24Hours();
      card.statsNext24Hours = srf.getStatsNext24Hours();
      card.mode = getMode(card.statsPast24Hours, card.statsNext24Hours);
      card.newCardsToday = srf.getCountNewCardsToday();
      const config = srf.getConfig();
      card.maxViewTime = config.maxViewTime;

      res.json(card);
    } else {
      res.status(404).send('no such card ID: ' + cardid);
    }
  });
