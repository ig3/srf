 ...
 *
 * Accurately modelling stud time in a window is complicated. Here, a crude
...
function getEstimatedAverageStudyTime (days) {
  ...
  console.log('estimated study time: ', days, cardsDue, revs, Math.floor(revs.time * cardsDue / revs.cards / days));
...
function getAverageStudyTimePerReview (days = 14) {
  const result = db.prepare('select avg(studytime) as t from revlog where id > ?')
...
function getChartStudyTime () {
  ...
  const revs =
    db.prepare('select count(distinct cardid) as cards, sum(studytime) as time from revlog where id > ?')
    .get((now() - secPerDay) * 1000);
  const timePerCard = revs.time / revs.cards / 60; // minutes
  console.log('timePerCard: ', timePerCard);
  const chart1Data = { x: [], y: [], type: 'bar' };
...
const getCorrectFactor = (function () {
  ...
  function update () {
    ...
    if (
      ...
      const change = Math.trunc(percentCorrect - config.percentCorrectTarget);
      if (change !== 0) {
