...
function getNextCard (overrideLimits) {
  if (overrideLimits) return getDueCard() || getNewCard();

  const studyTimePast24Hours = getStatsPast24Hours().time;

  if (
    studyTimePast24Hours < config.maxStudyTime
  ) {
    if (
      studyTimePast24Hours < config.minStudyTime &&
      getStatsNext24Hours().time < config.minStudyTime &&
      getCountNewCardsPast24Hours() < config.maxNewCardsPerDay &&
      getCountCardsOverdue() === 0 &&
      studyTimeSince(lastNewCardTime) >= newCardInterval()
    ) {
      return getNewCard();
    } else {
      return getDueCard();
...
/*
 * studyTimeSince returns the total study time (seconds) since the given time
 * (seconds since the epoch).
 */
function studyTimeSince (time) {
  return db.prepare(`
    select sum(studytime) as time
    from revlog
    where id > ?
  `)
  .get(time * 1000).time || 0;
}

/**
 * newCardInterval returns the seconds that should pass between new cards.
 *
 * The interval is the estimated study time over then next 24 hours divided
 * by the estimated number of cards required to bring study time up to
 * config.minStudyTime.
 */
function newCardInterval () {
  const studyTimeNext24Hours = getStatsNext24Hours().time;
  if (studyTimeNext24Hours < config.minStudyTime) {
    const timePerNewCard = getAverageStudyTimePerNewCard();
    const newCards = Math.min(
      config.maxNewCardsPerDay,
      (config.minStudyTime - studyTimeNext24Hours) / timePerNewCard
    );
    return Math.floor(studyTimeNext24Hours / newCards / 2);
  } else {
    return Math.floor(config.minStudyTime / config.maxNewCardsPerDay);
  }
}
...
function getDueCard (overrideLimits) {
  if (overrideLimits) {
    const cards = db.prepare('select * from card where interval != 0 order by due, templateid limit 5').all();
    return (cards[Math.floor(Math.random() * cards.length)]);
  } else if (Math.random() < config.probabilityOldestDue) {
...
/**
 * getAverageStudyTimePerNewCard returns the average study time per day per
 * new card studied. This considers only cards with interval less than one
 * day. These cards will typically be reviewed many times per day, with a
 * higher study time per card than for cards with longer intervals.
 *
 */
function getAverageStudyTimePerNewCard (days = 14) {
  ...
  let stats =
    db.prepare(`
      select
        sum(studytime) / count(distinct cardid) as average
      from revlog
      where
        interval < 68400 and
        id > ?
...
/**
 * getAverageStudyTimePerOldCard returns the average study time per day per
 * old card studied. This considers only cards with interval greater than one
 * day.
 */
...
function getNewCardsToReview (secs) {
  ...
  let cardsDue =
    db.prepare(`
      select
        count(distinct fieldsetid) as count
      from card
      where
        interval != 0 and
        interval < 68400 and
...
function getStatsNext24Hours () {
  const countCards = getCardsToReview(secPerDay);
  const countNewCards = getNewCardsToReview(secPerDay);
  const countOldCards = countCards - countNewCards;
  return ({
    count: countCards,
    time:
      countOldCards * getAverageStudyTimePerOldCard() +
      countNewCards * getAverageStudyTimePerNewCard()
  });
}
