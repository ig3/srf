...
function getNextCard (overrideLimits) {
  ...
  const minReviewsPerNewCard =
    statsPast24Hours.count / config.maxNewCardsPerDay / 2;

  if (
    statsPast24Hours.time < config.maxStudyTime
  ) {
    const dueCard = getDueCard();
    if (
      statsPast24Hours.time < config.minStudyTime &&
      getStatsNext24Hours().time < config.minStudyTime &&
      statsPast24Hours.newCards < config.maxNewCardsPerDay &&
      getCountCardsOverdue() === 0 && (
        reviewsSinceLastNewCard >= minReviewsPerNewCard ||
        (!dueCard && statsPast24Hours.time < config.minStudyTime / 2)
      )
    ) {
      return getNewCard();
    } else {
 ...
 * ease of the last review (fail, hard, good or easy). This is the
...
function getNewInterval (card, ease) {
  if (ease === 'fail') return intervalFail(card);
    ...
    Math.max(
      ...
      Math.floor(
        Math.min(
          ...
          card.interval * config.failFactor
        )
      )
    )
  );
...
function reviewCard (card, viewTime, studyTime, ease) {
  if (card.interval === 0) {
    reviewsSinceLastNewCard = 0;
  } else {
    reviewsSinceLastNewCard++;
  }
