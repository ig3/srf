...
function updateSeenCard (card, viewTime, studyTime, ease, interval) {
  ...
  const lapsed =
    ...
    card.lastinterval > config.matureThreshold;
  ...
  .run(now(), factor, interval, interval, due, card.views + 1, lapses, card.id);
...
function getPercentCorrect (
  ...
  const result = db.prepare("select count() as count, avg(case ease when 'again' then 0 else 1 end) as average from (select ease from revlog where lastinterval > ? and lastinterval < ? and id > ?)")
...
function updateDailyStats () {
  ...
  db.prepare(
    `select
      ...
      count() as cards
    from
      (select revdate from revlog group by cardid)
    where revdate >= ?
    group by revdate`
  )
  .all(dateLastUpdate)
  .forEach(row => {
    if (!data[row.revdate]) data[row.revdate] = {};
    data[row.revdate].newcards = row.cards;
  });
