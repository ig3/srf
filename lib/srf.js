...
function logReview (card, viewTime, studyTime, ease, factor, interval, dispersion, due, lapses) {
  const studyTimeToday = getStudyTimeToday() + viewTime;
  ...
  console.log(
    ...
    formatDue(timeLastSeen), // when card was last seen
...
function getDueCard () {
  ...
  let limit = now();
  if (limit - lastReviewTime < 300) limit += config.previewWindow;
  const card = db.prepare('select * from card where interval != 0 and due < ? order by interval, due, templateid limit 1').get(limit);
  return (card);
}
...
function buryRelated (card) {
  const due = now() + secPerDay * 5;
  const info = db.prepare('update card set due = ? where fieldsetid = ? and id != ? and due < ?')
  .run(
    due,
    card.fieldsetid,
    card.id,
...
function intervalAgain (card) {
  return (Math.floor(Math.max(config.againMinInterval, card.interval * 0.02)));
}

function intervalHard (card) {
  const timeLastSeen = getTimeCardLastSeen(card.id);
  if (!timeLastSeen) return (config.hardMinInterval);
  const timeSinceLastSeen = now() - timeLastSeen;
  const interval = Math.max(card.interval, timeSinceLastSeen);
  return (
    Math.max(
      config.hardMinInterval,
      Math.floor(interval * (1 - config.hardIntervalFactor / 100))
    )
  );
}
...
function intervalGood (card) {
  const timeLastSeen = getTimeCardLastSeen(card.id);
  if (!timeLastSeen) return (config.goodMinInterval);
  const timeSinceLastSeen = now() - timeLastSeen;
  const interval = Math.max(card.interval, timeSinceLastSeen);
  ...
  const factor = 1.5 +
    card.factor *
    (correctFactor / 1000) *
    Math.exp(-interval / 60 / 60 / 24 / 7);
  console.log('factor: ',
    factor.toFixed(2),
    card.factor.toFixed(2),
    (correctFactor / 1000).toFixed(3),
    Math.exp(-interval / 60 / 60 / 24 / 7).toFixed(2)
  );
  return (
    Math.min(
      config.maxInterval,
...
function intervalEasy (card) {
  const timeLastSeen = getTimeCardLastSeen(card.id);
  if (!timeLastSeen) return (config.easyMinInterval);
  const timeSinceLastSeen = now() - timeLastSeen;
  const interval = Math.max(card.interval, timeSinceLastSeen);
  const factor = 3.0 + card.factor * Math.exp(-interval / 60 / 60 / 24 / 7);
  return (
    Math.min(
      config.maxInterval,
      Math.max(
...
function getConfig (opts) {
  const defaults = {
    ...
    previewWindow: 60 * 60 * 4,

    // Backup retention time (milliseconds)
    backupRetention: 1000 * 60 * 60 * 24 * 7,
    ...
    correctFactorAdjustmentInterval: 60 * 60 * 24,
...
function reviewCard (card, viewTime, studyTime, ease) {
  ...
  if (ease === 'again') {
    ...
    updateSeenCard(card, viewTime, studyTime, 1, interval);
  } else if (ease === 'hard') {
    const interval = intervalHard(card);
    updateSeenCard(card, viewTime, studyTime, 2, interval);
  } else if (ease === 'good') {
    const interval = intervalGood(card);
    updateSeenCard(card, viewTime, studyTime, 3, interval);
  } else if (ease === 'easy') {
    const interval = intervalEasy(card);
    updateSeenCard(card, viewTime, studyTime, 4, interval);
  } else {
...
function updateCorrectFactor () {
  const correctFactor = getCorrectFactor();
  const percentCorrect = getPercentCorrect();
  const change = (percentCorrect < 89) ? -1 : (percentCorrect < 91) ? 0 : 1;
  if (change !== 0) {
    if (
      !db.prepare('select 1 from config where name = ?').get('correct factor')
    ) {
      db.prepare('insert into config (value, name) values (?, ?)')
      .run(
        (correctFactor + change).toString(),
        'correct factor'
      );
    } else {
      const info = db.prepare(
        'update config set value = ? where name = ? and modified < ?'
      )
      .run(
        (correctFactor + change).toString(),
        'correct factor',
        now() - config.correctFactorAdjustmentInterval
      );
      if (info.changes !== 0) {
        console.log('update correct factor: ', correctFactor + change);
      }
    }
  }
}
...
function fileOlderThanBackupRetention (path) {
  return (fs.statSync(path).mtime < (Date.now() - config.backupRetention));
}
