...
function updateSeenCard (card, elapsed, ease, interval) {
  // Add dispersion to the interval so cards don't keep appearing together.
  interval = Math.floor(interval *
    (config.intervalDispersionFactor - Math.random()) /
    config.intervalDispersionFactor);
  const factor = newFactor(card, interval);
  const now = Math.floor(Date.now() / 1000);
  let due = now + interval;
  if ((due - now) > config.dueTimeRoundingThreshold) {
    due = Math.floor(
      new Date(due * 1000).setHours(0, 0, 0, 0).valueOf() / 1000
    );
    interval = due - now;
  }
