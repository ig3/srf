...
function getWhyNotNewCard () {
  const newCardsPast24Hours = getCountNewCardsPast24Hours();
  if (newCardsPast24Hours >= config.newCardLimit) {
    return ('New cards past 24 hours: ' + newCardsPast24Hours + ' > ' +
      config.newCardLimit);
  }

  const overdueCards = getCountCardsOverdue();
  if (getCountCardsOverdue() > 0) {
    return ('Overdue cards: ' + overdueCards + ' > 0');
  }

  const averageStudyTimePastDay = getAverageStudyTime(1);
  if (averageStudyTimePastDay > config.studyTimeLimit) {
    return ('Study time past day: ' +
      averageStudyTimePastDay + ' > ' + config.studyTimeLimit);
  }

  const averageStudyTimePastTwoWeeks = getAverageStudyTime(14);
  if (averageStudyTimePastTwoWeeks > config.studyTimeLimit) {
    return ('Study time past two weeks: ' +
      averageStudyTimePastTwoWeeks + ' > ' + config.studyTimeLimit);
  }

  const estimatedStudyTimeNextDay = getEstimatedAverageStudyTime(1);
  if (estimatedStudyTimeNextDay > config.studyTimeLimit) {
    return ('Study time next day: ' +
      estimatedStudyTimeNextDay + ' > ' + config.studyTimeLimit);
  }

  const estimatedStudyTimeNextFiveDays = getEstimatedAverageStudyTime(5);
  if (estimatedStudyTimeNextFiveDays > config.studyTimeLimit) {
    return ('Study time next five days: ' +
      estimatedStudyTimeNextFiveDays + ' > ' + config.studyTimeLimit);
  }

  const percentCorrect = getPercentCorrect() || 100;
  if (percentCorrect < config.newCardMinPercentCorrect) {
  ...
  if (nextDueCard && timeSinceLastNewCard < minInterval) {
    return ('Too soon: ' + timeSinceLastNewCard + ' < ' + minInterval);
  }
