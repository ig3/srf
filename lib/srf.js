...
function getPercentCorrect (
  ...
  return ((result && result.average) ? result.average * 100 : config.percentCorrectTarget);
}
...
function importAnki21 (opts, data) {
  ...
  const insertFieldset = db.prepare('insert into fieldset (guid, templateset, fields) values (?,?,?)');
  const updateFieldset = db.prepare('update fieldset set templateset = ?, fields = ? where id = ?');
  const notes = {};
  ...
  .forEach(record => {
    ...
    if (fieldsetGuidToId[record.guid]) {
      updateFieldset
      .run(model.srfTemplateset, JSON.stringify(fields), fieldsetGuidToId[record.guid]);
      record.srfFieldsetid = fieldsetGuidToId[record.guid];
    } else {
      const info = insertFieldset
      .run(record.guid, model.srfTemplateset, JSON.stringify(fields));
 ...
 */
function createFieldset (guid, templateset, fields) {
  const info = db.prepare('insert into fieldset (guid, templateset, fields) values (?, ?, ?)')
  .run(
    guid,
    templateset,
    fields
...
function updateFieldset (templateset, fields, fieldsetid) {
  ...
  db.prepare('update fieldset set templateset = ?, fields = ? where id = ?')
  .run(templateset, fields, fieldsetid);
  createCards(fieldsetid, templateset);
}
