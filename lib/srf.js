 ...
 *
 * It's tempting to just return minStudyTime / maxNewCardsPerDay but
 * initially there is nothing to study so not enough new cards will be
 * presented. So if predicted study time is less than minStudyTime we need
 * to present new cards at shorter intervals.
 *
 * Present all the new cards in the first half of the day, so there is time
 * to review them a few times before the end of the day.
 */
function newCardInterval () {
  const studyTime = Math.min(
    config.minStudyTime,
    getStatsNext24Hours().time
  );
  return Math.floor(studyTime / 2 / config.maxNewCardsPerDay);
}
