...
function updateSeenCard (card, viewTime, studyTime, ease, interval) {
  ...
  buryRelated(card);
  logReview(card, viewTime, studyTime, ease, factor, interval, due, lapses);
}

function logReview (card, viewTime, studyTime, ease, factor, interval, due, lapses) {
...
function purgeOldDatabaseBackups() {
  ...
  .filter(path => fileOlderThanBackupRetention(path))
  .forEach(path => fs.unlinkSync(path));
}

function fileOlderThanBackupRetention (path) {
  return (fs.statSync(path).mtime < (Date.now() - opts.backupRetention));
}
