...
function updateSeenCard (card, viewTime, studyTime, ease, interval) {
  const factor = newFactor(card, ease);
...
function intervalGood (card) {
  ...
  const timeSinceLastSeen = now() - timeLastSeen;
  const interval = Math.max(card.interval, timeSinceLastSeen);
  const factor = getGoodFactor(card, interval);
  return (
    Math.min(
      config.maxInterval,
      Math.max(
...
function getGoodFactor (card, interval) {
  return card.factor * (getCorrectFactor() / 1000);
}
...
function intervalEasy (card) {
  ...
  const timeSinceLastSeen = now() - timeLastSeen;
  const interval = Math.max(card.interval, timeSinceLastSeen);
  const factor = 1.5 * getGoodFactor(card, interval);
  return (
    Math.min(
      config.maxInterval,
      Math.max(
...
/**
 * newFactor returns a decaying average of the ease.
 */
function newFactor (card, ease) {
  const easeMap = {
    again: 0,
    hard: 1,
    good: 2,
...
const getCorrectFactor = (function () {
  ...
  function update () {
    ...
    if (
      ...
      const change = (percentCorrect < 89) ? -1 : (percentCorrect < 92) ? 0 : 1;
      if (change !== 0) {
