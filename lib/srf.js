...
function prepareDatabase () {
  const currentSchemaVersion = 6; // This is the latest schema version
  let schemaVersion;
  try {
    const result =
...
function getTemplateSetIdFromAnkiModel (keyMap, model) {
  ...
  const info = createTemplateset(model.name);
...
function createTemplateset (name) {
  const info = db.prepare('insert into templateset (name) values (?)')
  .run(name);
  return (info);
}

function updateTemplateset (name, id) {
  db.prepare('update templateset set name = ? where id = ?')
  .run(
    name,
    id
  );
