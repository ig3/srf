...
function getChartCardsStudiedPerDay () {
  ...
  let first;
  const data = {};
  db.prepare("select strftime('%Y-%m-%d', id / 1000, 'unixepoch', 'localtime') as day, count() as cards from revlog group by day").all().forEach(el => {
    if (!first) first = el.day;
    data[el.day] = el.cards;
  });
  ...
  while (1) {
    // Get the date as a string in localtime
    const date = new Date(d.getTime() - (d.getTimezoneOffset() * 60000))
      .toISOString()
      .split('T')[0];
    chart1Data.x.push(date);
    chart1Data.y.push(data[date] || 0);
    if (date >= today) break;
...
function getChartMaturedAndLapsedPerDay () {
  ...
  const data = {};
  let first;
  db.prepare("select strftime('%Y-%m-%d', id / 1000, 'unixepoch', 'localtime') as day, count(case when interval >= @threshold and lastinterval < @threshold then 1 else null end) as matured, count(case when interval < @threshold and lastinterval >= @threshold then 1 else null end) as lapsed from revlog group by day")
  .all({
    secPerDay: secPerDay,
  ...
  while (1) {
    ...
    const date = new Date(d.getTime() - (d.getTimezoneOffset() * 60000))
      ...
      .split('T')[0];
    const x = data[date];
    chart6Trace1.x.push(date);
    chart6Trace1.y.push(x ? x.matured - x.lapsed : 0);
    chart6Trace2.x.push(date);
    chart6Trace2.y.push(x ? x.lapsed : 0);
    chart6Trace3.x.push(date);
    chart6Trace3.y.push(x ? x.matured : 0);
    total += x ? x.matured - x.lapsed : 0;
    chart6Trace4.x.push(date);
    chart6Trace4.y.push(total);
    if (date >= today) break;
    d.setDate(d.getDate() + 1);
  }
...
function getChartMinutesStudiedPerDay () {
  ...
  const data = {};
  let first;
  db.prepare("select strftime('%Y-%m-%d', id / 1000, 'unixepoch', 'localtime') as day, sum(studytime) as time from revlog group by day").all().forEach(el => {
    if (!first) first = el.day;
    data[el.day] = el.time / 60;
  });
  ...
  while (1) {
    ...
    const date = new Date(d.getTime() - (d.getTimezoneOffset() * 60000))
      .toISOString()
      .split('T')[0];
    chart2Data.x.push(date);
    chart2Data.y.push(data[date] || 0);
    if (date >= today) break;
    d.setDate(d.getDate() + 1);
  }
  return (chart2Data);
...
function getChartNewCardsPerDay () {
  ...
  const data = {};
  let first;
  db.prepare("select strftime('%Y-%m-%d', id / 1000, 'unixepoch', 'localtime') as day, count() as cards from (select id from revlog group by cardid) group by day").all().forEach(el => {
    if (!first) first = el.day;
    data[el.day] = el.cards;
  });
  ...
  while (1) {
    ...
    const date = new Date(d.getTime() - (d.getTimezoneOffset() * 60000))
      .toISOString()
      .split('T')[0];
    chart5Data.x.push(date);
    chart5Data.y.push(data[date] || 0);
    if (date >= today) break;
    d.setDate(d.getDate() + 1);
  }
  return (chart5Data);
...
function adjustCards () {
  ...
  if (Math.abs(error) > 1) {
    ...
    console.log('adjustCards: ', adjustment);
    db.prepare('update card set interval = floor(interval + interval * @adjustment), due = floor(due + (due - @now) * @adjustment) where due > @now and interval > @minInterval and interval < @maxInterval')
    .run({
