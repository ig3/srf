...
function newCardInterval () {
  ...
  if (newCardsToStudy <= 1) return (timeToStudyCurrentlyDueCards());
  return (Math.floor(timeToStudyCurrentlyDueCards() / (newCardsToStudy - 1)));
}
 ...
 *
 * Do not present a new card if there is a card from the same template due
...
function getNewCard () {
  ...
  .get(now() + config.minTimeBetweenRelatedCards);
  return (card);
}
 ...
 *
 * The current new card limit is config.newCardLimit, reduced if average
 * study time exceeds 90% of config.studyTimeLimit, down to 0 if average
 * study time exceeds 110% of config.studyTimeLimit.
 *
 ...
 *
 * Limit is 0 if study time the past 24 hours or estimated study time the
 * next 24 hours is more than config.studyTimeLimit.
 */
function getCurrentNewCardLimit () {
  ...
  return Math.min(
    ...
    Math.max(
      ...
      Math.floor(
        config.newCardLimit * (
          // 90% to 110%, a range of 20%
          (110 - 100 * averageStudyTime / config.studyTimeLimit) / 20
        )
      )
    )
  );
