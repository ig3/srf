...
function updateSeenCard (card, viewTime, studyTime, ease, interval) {
  ...
  const due = Math.floor(now() + interval);
...
function getNextCard () {
  ...
  const newCardsPast24Hours = getCountNewCardsPast24Hours();
  const currentNewCardLimit = getCurrentNewCardLimit();
  if (newCardsPast24Hours < currentNewCardLimit) {
    if (
      (now() - lastNewCardTime) > newCardInterval() || !nextDueCard
    ) {
      return (getNewCard());
    }
  }
...
/**
 * newCardInterval returns the time that should pass between new cards.
 *
 * So that new cards are evenly spaced, the estimated total time to study
 * all cards currently due is divided by the number of new cards that may
 * currently be studied.
 */
function newCardInterval () {
  const newCardsToStudy =
    getCurrentNewCardLimit() - getCountNewCardsPast24Hours();
  if (newCardsToStudy === 0) return 0;
  return (Math.floor(timeToStudyCurrentlyDueCards() / newCardsToStudy));
}

function timeToStudyCurrentlyDueCards () {
  return (getCountCardsDueNow() * getAverageStudyTimePerReview());
}
...
function getNewCard () {
  ...
  .get(
    now() + config.minTimeBetweenRelatedCards, // seconds
    (now() - config.minTimeBetweenRelatedCards) * 1000 // milliseconds
  );
  return (card);
}

/**
 * getCurrentNewCardLimit returns the current limit on new cards.
 *
 * The current new card limit is a linear function of average study time,
 * except that it is clipped at a lower bound of 0 and an upper bound of
 * config.newCardLimit. Otherwise, it varies with slope determined by
 * config.studyTimeSensitivity. When average study time is equal to
 * config.studyTimeLimit, the current new card limit is 50% of
 * config.newCardLimit.
 *
 * Study time is averaged over the past week (actual study times) and the
 * upcoming week (estimated, based on recent performance and cards due).
 *
 */
function getCurrentNewCardLimit () {
  if (getCountCardsOverdue() > 0) return (0);
  if (getStatsPast24Hours().time > config.studyTimeLimit) return (0);
  if (getEstimatedAverageStudyTime(1) > config.studyTimeLimit) return (0);
  const averageStudyTime =
    (getEstimatedAverageStudyTime(7) + getAverageStudyTime(7)) / 2;
  const ratio = averageStudyTime / config.studyTimeLimit;
  return Math.min(
    config.newCardLimit,
    Math.max(
      0,
      Math.floor(
        config.newCardLimit * (0.5 + config.studyTimeSensitivity * (1 - ratio))
      )
    )
  );
}
 ...
 */
function getDueCard () {
  if (Math.random() < config.probabilityOldestDue) {
    const cards = db.prepare('select * from card where interval != 0 and due < ? order by due, templateid limit 5').all(now());
    return (cards[Math.floor(Math.random() * cards.length)]);
  } else {
    const cards = db.prepare('select * from card where interval != 0 and due < ? order by interval, due, templateid limit 5').all(now());
    return (cards[Math.floor(Math.random() * cards.length)]);
  }
}
 ...
 * day, considering cards due in the given window of days.
 *
...
function getEstimatedAverageStudyTime (days) {
  const cardsDue = getCardsToReview(days * secPerDay);

  // Check history past days
  let revs =
  ...
  if (revs && revs.cards > 0) {
    return Math.floor(cardsDue * revs.time / revs.cards / days);
  }

  // If no histary in past days, check all history
  revs =
...
/**
 * getCardsToReview returns the number of cards to be reviewed in the next
 * secs seconds. It does not count cards that will be deferred.
 */
function getCardsToReview (secs) {
  // Below config.minTimeBetweenRelatedCards, count one card per fieldset.
  const limit1 = Math.min(secs, config.minTimeBetweenRelatedCards);
  let cardsDue =
    db.prepare('select count(distinct fieldsetid) as count from card where interval != 0 and due <= ?')
    .get(now() + limit1).count;

  // Above config.minTimeBetweenRelatedCards, count every card.
  if (secs > limit1) {
    cardsDue +=
      db.prepare('select count() as count from card where interval != 0 and due > ? and due < ?')
      .get(now() + limit1, now() + secs).count;
  }
  return cardsDue;
}
...
function getCountCardsDueToday () {
  ...
  return (getCardsToReview(endOfDay - now()));
}
...
/**
 * getCountNewCardsRemaining returns the difference between the current new
 * card limit and the number of new cards seen in the past 24 hours.
 */
function getCountNewCardsRemaining () {
  return Math.max(0, getCurrentNewCardLimit() - getCountNewCardsPast24Hours());
}
...
function getStatsNext24Hours () {
  const count = getCardsToReview(secPerDay);
  return ({
    count: count,
...
function getAverageStudyTime (days = 14) {
  const results = db.prepare(
    `select sum(studytime) as studytime
     from revlog
     where id > ?`
  ).get((now() - days * secPerDay) * 1000);
  if (results && results.studytime) {
    return (Math.floor(results.studytime / days));
  } else {
    return 0;
  }
}
