...
function setCorrectFactor (factor) {
  const result = db.prepare('select modified from config where name = ?')
    .get('correct factor');
  if (result) {
    const now = Math.floor(Date.now() / 1000);
    if (result.modified < now - config.correctFactorAdjustmentInterval) {
      console.log('update config correct factor');
      db.prepare('update config set value = ? where name = ?')
      .run(factor.toString(), 'correct factor');
    }
  } else {
    console.log('insert config correct factor');
