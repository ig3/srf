...
const api = {
  ...
  createCard,
  createCards,
  createCardsForTemplate,
  deleteCardsForFieldset,
  deleteCardsForTemplate,
  getAverageStudyTime,
...
function updateSeenCard (card, elapsed, ease, interval) {
  ...
  buryRelated(card);
  logReview(card, elapsed, ease, factor, due, lapsed, lapses);
}

function logReview (card, elapsed, ease, factor, due, lapsed, lapses) {
 ...
 */
function getNextDue () {
  return db.prepare('select due from card where interval != 0 order by due limit 1').get().due;
}
...
function getCountMatureCards () {
  return (db.prepare('select count() from card where interval > 364*24*60*60').get()['count()'] || 0);
}
...
function getCountDaysStudied () {
  ...
  const first = db.prepare('select id from revlog limit 1').get().id;
...
function getPercentCorrect (n = 1000, minInterval = 60 * 60 * 24 * 2) {
  const result = db.prepare('select avg(case ease when 1 then 0 else 1 end) as average from (select ease from revlog where interval > ? order by id desc limit ?)')
  .get(minInterval, n);
...
function insertFieldset (guid, templatesetid, fields) {
  return (
    db.prepare('insert into fieldset (guid, templatesetid, fields) values (?, ?, ?)')
    .run(
      guid,
      templatesetid,
      fields
...
function insertTemplateset (name, fields) {
  const info = db.prepare('insert into templateset (name, fields) values (?, ?)')
  .run(
    name,
    fields
...
function insertTemplate (templatesetid, name, front, back, css) {
  db.prepare('begin transaction').run();
