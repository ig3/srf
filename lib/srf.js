...
module.exports = (options = {}) => {
  ...
  console.log('opts.database: ' + opts.database);
...
function shutdown () {
  ...
  .then(() => {
    if (this.backupIntervalID) {
      console.log('stop backup interval timer');
      clearInterval(this.backupIntervalID);
    }
  })
  .then(() => {
    if (this.server) {
      console.log('close the express server');
      this.server.close();
    }
  })
  .then(() => {
    if (this.scheduler) {
      console.log('shutdown scheduler');
  ...
  .then(() => {
    console.log('scheduler shut down. Now close the app');
    this.close();
...
function runServer (options, args) {
  ...
  server.on('error', err => {
    console.error('server threw an error: ', opts.verbose ? err : err.message);
    self.shutdown();
  });
}
