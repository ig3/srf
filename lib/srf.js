...
/**
 * getNewDue returns the new due time as seconds past the epoch.
 *
 * The due time is quantized to multiples of 1% of the interval.
 *
 * The due time has a uniformly distributed random disperson added that is
 * over the range
 *   [
 *     -config.dispersionFactor / 100 * interval,
 *     config.dispersonFactor / 100 * interval
 *   ],
 * where config.dispersionFactor is percentage
 */
function getNewDue (interval) {
  return (Math.floor(
    (
      Math.floor(
        (now() + interval) / (interval / 100)
      ) + Math.floor(
        Math.random() * (2 * config.dispersionFactor + 1) -
        config.dispersionFactor
      )
    ) * (interval / 100)
  ));
}
