...
Mustache.escape = function (text) {
  if (/\[sound:.*\]/.test(text)) {
    ...
    let result = '<audio id="myaudio" autoplay controls';
    result += ' data-files="' + src.join(',') + '"';
    result += '></audio>';
    return result;
  } else {
    return text;
...
/**
 * getNextDue returns the due time of card with the earliest due time
 */
function getNextDue () {
...
function getCardCounts () {
  return (db.prepare(`
    ...
    from card
  `).get({
    t1: config.stage1MaxInterval,
    t2: config.stage2MaxInterval,
    t3: config.stage3MaxInterval,
  }) || {
    total: 0,
    unseen: 0,
    seen: 0,
...
function getCountCardsStage1 () {
  return (
    ...
    .get(config.stage1MaxInterval)['count()'] || 0
  );
}
...
function getCountCardsStage2 () {
  return (
    ...
    .get(config.stage1MaxInterval, config.stage2MaxInterval)['count()'] || 0
  );
}
...
function getCountCardsStage3 () {
  return (
    db.prepare('select count() from card where interval >= ? and interval < ?')
    .get(config.stage2MaxInterval, config.stage3MaxInterval)['count()'] || 0
  );
}
...
function getCountCardsStage4 () {
  return (
    ...
    .get(config.stage3MaxInterval)['count()'] || 0
  );
}

/**
 * getCountCardsOverdue returns the number of cards that were due more than
 * one day before the given time. This is not when they were due at the
 * time, if they have been reviewed since that time. But this function
 * should only be used to calculate cards overdue on days after which there
 * has been no study. See updateDailyStats.
 */
function getCountCardsOverdue (
  ...
  return (
    ...
    .get(on - secPerDay)['count()'] || 0
  );
}

/**
 * getCountCardsDue returns the count of cards that were due before the
 * given time. This is not the time they were due at the given time, if
 * they have been reviewed since the given time. But this function should
 * only be used to calculate cards due on days when there was no study.
 * See updateDailyStats.
 */
function getCountCardsDue (
  on = now()
) {
  return (
    db.prepare('select count() from card where interval != 0 and due < ?')
...
function getDaysStudied () {
  const result = db.prepare('select min(id) as first from revlog').get();
  if (result && result.first) {
    return Math.max(1, ((Date.now() - result.first) / 86400000));
  } else {
    return 0;
  }
}
...
function getAverageHistoricLatency (days = 28) {
  let n = 0;
  let total = 0;
  db.prepare(`
    select
...
function getAverageLatency (days = 28) {
  const cNow = now();
  let n = 0;
  let total = 0;
  db.prepare(`
...
function getConfig (opts) {
  if (!config) {
    const defaults = {
      ...
      // Minimum number of reviews in
      percentCorrectMinCount: 10,
      percentCorrectWindow: '1 month',

      // This is the maximum interval for 'new' cards. It has no functional
      // impact but affects statistics on the stats page.
      stage1MaxInterval: '1 week',
      // This is the maximum interval for 'learning' cards.
      stage2MaxInterval: '21 days',
      // This is the maximum interval for 'mature' cards.
      // It should be the same as the scheduler's maximum interval.
      // Cards are not expected to have an interval greater than this.
...
/**
 * resetDatabase does a reset of the scheduler
 *  - resets all cards
 *  - clears review history
 */
function resetDatabase () {
  console.log('reset database');
  const result = this.scheduler.reset();
  return result;
}
...
function getPercentCorrect (
  ...
  minInterval = config.stage2MaxInterval,
  maxInterval = config.stage3MaxInterval
) {
  const result = db.prepare(
    `select
      count() as count,
  ...
  if (result && result.count > config.percentCorrectMinCount) {
...
function getChartCardsDuePerDay () {
  ...
  const points = [];
  let last;
  const first = Math.floor((startOfDay - timezoneOffset) / (60 * 60 * 24));
...
function getChartCardsPerInterval () {
  ...
  if (last === Math.floor(config.stage3MaxInterval / 60 / 60 / 24)) {
    last--;
  }
  const chart = {
    x: [],
...
function getChartStudyTime () {
  ...
  db.prepare(`
    select
      cast((id / 1000 - @start)/(60*60) as integer) as hour,
      sum(case when lastinterval != 0 then studytime else 0 end) as time,
      sum(case when lastinterval = 0 then studytime else 0 end) as newtime
    from revlog
    where id / 1000 > @start
    group by hour
  `)
  .all({ start: now() - secPerDay + 1 }) // +1 to capture review this second
  .forEach(el => {
    reviewtime[el.hour] = el.time / 60;
    newtime[el.hour] = el.newtime / 60;
  });

  for (let i = 0; i < 24; i++) {
    chart1Data.x.push(i - 24);
    chart1Data.y.push(reviewtime[i] || 0);
    chart2Data.x.push(i - 24);
    chart2Data.y.push(newtime[i] || 0);
  }
  ...
  .all({ start: now(), end: now() + secPerDay })
  .forEach(el => {
    reviewtime[el.hour] = el['count()'] * timePerCard;
  });
  reviewtime[0] = (reviewtime[0] || 0) + getCountCardsDue() * timePerCard;
...
/**
 * getChartsLatency returns a set of chart data from dailystats table
 * Latency, backlog and overdue.
 */
function getChartsLatency () {
  const chartLatency = { x: [], y: [] };
  const chartBacklog = { x: [], y: [] };
  const chartOverdue = { x: [], y: [] };
  db.prepare(
    `select
      date,
      latency,
  ...
  .all()
  .forEach(row => {
    chartLatency.x.push(row.date);
    chartLatency.y.push(Math.floor((row.latency || 0) / 60));
    chartBacklog.x.push(row.date);
    chartBacklog.y.push(row.backlog || 0);
    chartOverdue.x.push(row.date);
    chartOverdue.y.push(row.overdue || 0);
  });

  return ({
    chartLatency: chartLatency,
    chartBacklog: chartBacklog,
    chartOverdue: chartOverdue,
  });
}
...
function updateDailyStats () {
  ...
  if (lastRow) {
    ...
    while (d < date) {
      const on = Math.floor(new Date(d).valueOf() / 1000);
      const percentCorrect = getPercentCorrect(on);
      const backlog = getCountCardsDue(on);
      const overdue = getCountCardsOverdue(on);
      db.prepare(
  ...
  const d1 = db.prepare(
    `select
       ...
       cast(avg(id/1000 - lastdue) as int) as latency,
       max(backlog) as backlog,
  ...
  .run({
    ...
    stageMature: stageMature,
    stageMastered: stageMastered,
    latency: d1.latency || 0,
    backlog: d1.backlog || 0,
    overdue: d1.overdue || 0,
  });
}
    ...
    if (
      ...
      Object.keys(cards).forEach(key => {
        ...
        if (currentCards[cards[key].id] || due > row.date) {
          if (cards[key].interval < config.stage1MaxInterval) {
            stageNew++;
          } else if (cards[key].interval < config.stage2MaxInterval) {
            stageLearning++;
          } else if (cards[key].interval < config.stage3MaxInterval) {
            stageMature++;
          } else {
            stageMastered++;
          }
...
function runServer (options, args) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Despite Express documentation, app.listen() is not identical to
    // http.Server.listen(). In particular, the callback to app.listen is
    // wrapped to ensure it is not called more than once and the wrapped
    // function is attached to the http.Server error AND listening events.
    // http.Server.listen() only attaches it to the listening event.
    // It is simpler to attach handlers to these events explicitly.
