...
function updateSeenCard (card, elapsed, ease, interval) {
  ...
  let due = now + interval *
    (config.dispersionFactor - Math.random()) /
...
function getCountMatureCards () {
  return (
    ...
    .get(config.maxInterval - secPerDay)['count()'] || 0
  );
}
...
function getPercentCorrect (n = 1000, minInterval = 60 * 60 * 24 * 2) {
  const result = db.prepare('select avg(case ease when 1 then 0 else 1 end) as average from (select ease from revlog where lastinterval > ? order by id desc limit ?)')
  .get(minInterval, n);
...
function getChartMaturedAndLapsedPerDay () {
  ...
  const points = [];
  let first = null;
  db.prepare('select cast((id - @tzOffset)/1000/@secPerDay as int) as day, count(case when interval >= @maxInterval and lastinterval < @maxInterval then 1 else null end) as matured, count(case when interval < @maxInterval and lastinterval >= @maxInterval then 1 else null end) as lapsed from revlog group by day')
  .all({
    tzOffset: timezoneOffset * 1000,
    secPerDay: secPerDay,
    maxInterval: config.maxInterval - secPerDay
  })
  .forEach(el => {
    if (!first) first = el.day;
    points[el.day - first] = {
...
function reviewCard (card, elapsed, ease) {
  if (elapsed > 120) {
    // Just ignore it: the card will be presented again
  } else if (ease === 'again') {
