...
function intervalGood (card) {
  ...
  console.log('factor: ',
    ...
    Math.exp(-timeSinceLastSeen / 60 / 60 / 24 / 7).toFixed(2)
  );
  return (
    Math.min(
      config.maxInterval,
...
function getChartStudyTime () {
  ...
  const chart1Data = { x: [], y: [], type: 'bar' };
  const result1 = db.prepare('select cast((id / 1000 - @start)/(60*60) as integer) as hour, sum(studytime) as time from revlog where id / 1000 > @start group by hour')
...
function purgeOldDatabaseBackups() {
  const backups = fs.readdirSync(opts.dir)
