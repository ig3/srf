 ...
 * study time exceeds 95% of config.studyTimeLimit, down to 0 if average
 * study time exceeds 105% of config.studyTimeLimit.
 *
...
function getCurrentNewCardLimit () {
  ...
  return Math.min(
    ...
    Math.max(
      ...
      Math.floor(
        config.newCardLimit * (
          10.5 - 10 * averageStudyTime / config.studyTimeLimit
        )
      )
    )
  );
