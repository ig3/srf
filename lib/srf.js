...
function getChartCardsStudiedPerDay () {
  ...
  let first;
  const data = {};
  db.prepare("select revdate, count() as cards from revlog group by revdate").all().forEach(el => {
    if (!first) first = el.revdate;
    data[el.revdate] = el.cards;
  });
...
function getChartMaturedAndLapsedPerDay () {
  ...
  const data = {};
  let first;
  db.prepare("select revdate, count(case when interval >= @threshold and lastinterval < @threshold then 1 else null end) as matured, count(case when interval < @threshold and lastinterval >= @threshold then 1 else null end) as lapsed from revlog group by revdate")
  .all({
    secPerDay: secPerDay,
...
function getChartMinutesStudiedPerDay () {
  ...
  const data = {};
  let first;
  db.prepare("select revdate, sum(studytime) as time from revlog group by revdate").all().forEach(el => {
    if (!first) first = el.revdate;
    data[el.revdate] = el.time / 60;
  });
...
function getChartNewCardsPerDay () {
  ...
  const data = {};
  let first;
  db.prepare("select revdate, count() as cards from (select revdate from revlog group by cardid) group by revdate").all().forEach(el => {
    if (!first) first = el.revdate;
    data[el.revdate] = el.cards;
  });
