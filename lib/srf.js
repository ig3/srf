 ...
 *
 * The due time is quantized according to the interval
 */
function getNewDue (interval) {
  if (interval > 864000) {  // 10 days
    // quantize to the day
    return (Math.floor((now() + interval) / 86400) * 86400);
  } else if (interval > 86400) {  // 1 day
    // quantize to the hour
    return (Math.floor((now() + interval) / 3600) * 3600);
  } else {
