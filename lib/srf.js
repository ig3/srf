...
function getNextCard () {
  ...
  const timeSinceLastNewCard = now() - lastNewCardTime;
  const nextDueCard = getDueCard();
  const minInterval = Math.floor(config.studyTimeLimit / config.newCardLimit);
  if (
    newCardsPast24Hours >= currentNewCardLimit ||
    (timeSinceLastNewCard < minInterval && nextDueCard)
  ) {
    return (nextDueCard);
  }
  return (getNewCard());
