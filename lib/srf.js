...
function getNextCard (overrideLimits) {
  ...
  const targetTime = (config.minStudyTime + config.maxStudyTime) / 2;
  ...
  if (
    ...
    if (
      statsPast24Hours.time < targetTime &&
      getStatsNext24Hours().time < targetTime &&
      statsPast24Hours.newCards < config.maxNewCardsPerDay &&
      getCountCardsOverdue() === 0 && (
        reviewsSinceLastNewCard >= minReviewsPerNewCard ||
        (!dueCard && statsPast24Hours.time < config.minStudyTime)
      )
    ) {
      return getNewCard();
    } else {
